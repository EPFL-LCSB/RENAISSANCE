...
def getindconstr(env, lp, which):
    ...
    if surplus == 0:
        return ([], [])
    # inout_list contains the space and which args to CPXXgetindconstr.
    inout_list = [surplus, which]
    status = CR.CPXXgetindconstr(env, lp, inout_list)
    check_status(env, status)
    # We expect to get [linind, linval]
    assert len(inout_list) == 2
    return tuple(inout_list)
...
def getindconstr_constant(env, lp, which):
    ...
    if status != CR.CPXERR_NEGATIVE_SURPLUS:
        check_status(env, status)
    # We expect to get [indvar, complemented, rhs, sense, surplus]
    assert len(inout_list) == 5
    assert len(inout_list[3]) == 1  # sense string should be one char
    inout_list[3] = cpx_encode(inout_list[3], default_encoding)
    return tuple(inout_list)
